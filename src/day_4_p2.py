...
def check_winner(boards, states, winners):
    all_winners = False
    last_index = None
    ...
    for i in range(0, len(states)):
        ...
        if sum(winners) == len(winners):
            all_winners = True
            last_index = i
            return all_winners, last_index, winners

    return all_winners, last_index, winners


def play_bingo(calls, boards, states, winners):
    # loop through calls until a we have a winner
    all_winners = False
    for call in calls:
        states = make_call(call, boards, states)
        all_winners, last_index, winners = check_winner(boards, states, winners)

        print(sum(winners),'winners out of ',len(winners),'total boards')

        if all_winners:
            return last_index, call
...
if __name__ == '__main__':
    calls = get_calls()
    boards, states, winners = get_boards()

    last_winning_index, last_winning_call = play_bingo(calls, boards, states, winners)
    last_winning_board = boards[last_winning_index]
    last_winning_state = states[last_winning_index]

    score = calculate_score(last_winning_call, last_winning_board, last_winning_state)

    print('*~*~*~*~*')
    print('Last winning call:', last_winning_call)
    ...
    print('*~*~*~*~*')
    print('Last winning state:')
    for row in last_winning_state:
        print(row)
